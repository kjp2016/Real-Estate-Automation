...
# ---------------------------
# NEW FUNCTION: Apply Original Header Order
# ---------------------------
def apply_original_header_order(merged_data: List[Dict[str, str]], original_header: List[str]) -> List[Dict[str, str]]:
    ordered_data = []
    for row in merged_data:
        # Build a new dictionary following the original header order.
        ordered_row = {col: row.get(col, "") for col in original_header}
        # Append any new columns (if desired) at the end.
        for col in row:
...
def process_files(compass_file: str, phone_file: str, mls_files: List[str], output_dir: str, logger=None):
    ...
    if not original_compass_data:
        ...
        return
    # Save the exact order from the original Compass export:
    original_field_order = list(original_compass_data[0].keys())
    ...
    # 2) Now apply the original header order to the final merged data.
    final_data_ordered = apply_original_header_order(final_data, original_field_order)

    # 3) Write the final merged data using the original header order.
    try:
        with open(merged_file, mode="w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=original_field_order)
            writer.writeheader()
            writer.writerows(final_data_ordered)
        if logger:
            logger(f"Final merged data with original column order written to {merged_file}")
    except Exception as e:
        if logger:
            logger(f"Error saving final merged CSV: {e}")

    # 4) Export only the updated records into import files.
    export_updated_records_with_fieldnames(merged_file, os.path.join(output_dir, "compass_import"), original_field_order, logger=logger)


# ---------------------------
# EXPORT UPDATED RECORDS FUNCTION (modified to accept final fieldnames)
# ---------------------------
def export_updated_records_with_fieldnames(merged_file: str, import_output_dir: str, final_fieldnames: List[str], logger=None):
    ...
    # Filter out excluded columns but preserve the order from final_fieldnames.
